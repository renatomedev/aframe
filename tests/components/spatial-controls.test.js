...
suite('spatial-controls', function () {
  var component;
  var controlsSystem;
  var el;
  ...
  setup(function (done) {
    ...
    el.addEventListener('componentinitialized', function (evt) {
      if (evt.detail.name !== 'spatial-controls') { return; }
      component = el.components['spatial-controls'];
      component.controllersWhenPresent = [
        {id: 'Spatial Controller (Spatial Interaction Source) ____-____', index: 0, hand: 'right', pose: {}}
      ];
      controlsSystem = el.sceneEl.systems['tracked-controls'];
      done();
    });
    el.setAttribute('spatial-controls', 'hand: right; model: false');  // To ensure index = 0.
  });
